using Application.Service.Interface;
using Consumer.Consumers;
using Core.Entity;
using Core.Message.Command;
using MassTransit;
using MassTransit.Testing;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Logging;
using Moq;

namespace IntegrationTest.Consumers;
public class CreateContactConsumerTests 
{
    [Fact]
    [Trait("Category", "Integration")]
    public async Task Consumer_ReceivesCommand_CallsContactServiceCreateAsync()
    {
        // Arrange
        var services = new ServiceCollection();
        var mockContactService = new Mock<IContactService>();

        mockContactService.Setup(s => s.CreateAsync(It.IsAny<Contact>())).Returns(Task.CompletedTask);
        services.AddSingleton(mockContactService.Object);

        services.AddSingleton(typeof(ILogger<>), typeof(Logger<>));

        services.AddMassTransitTestHarness(x =>
        {
            x.AddConsumer<CreateContactConsumer>();

            // Configure an in-memory receive endpoint where the consumer will listen
            x.UsingInMemory((context, cfg) =>
            {
                // Or if you explicitly named your queue:
                cfg.ReceiveEndpoint("contact-queue", e =>
                {
                    e.ConfigureConsumer<CreateContactConsumer>(context);
                });
            });
        });

        await using var provider = services.BuildServiceProvider();
        var harness = provider.GetRequiredService<ITestHarness>();
        await harness.Start();

        var command = new CreateContactCommand
        (
            "Test User 1",
            "99983-1617",
            "testUser1@gmail.com",
            11
        );

        // Act
        // Publish the command to the in-memory bus.
        // This simulates the producer sending the message.
        await harness.Bus.Publish(command);

        // Assert
        // Verify that the consumer received the message
        Assert.True(await harness.Consumed.Any<CreateContactCommand>());

        mockContactService.Verify(
                s => s.CreateAsync(It.Is<Contact>(c =>
                    c.Name == command.Name &&
                    c.Email == command.Email)),
                Times.Once());        

        // Stop the TestHarness
        await harness.Stop();
    }
}
