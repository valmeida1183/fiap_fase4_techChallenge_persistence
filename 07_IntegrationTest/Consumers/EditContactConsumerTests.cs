...
public class EditContactConsumerTests
{
    ...
    public async Task Consumer_ReceivesCommand_CallsContactServiceEditAsync()
    {
        // Arrange
        ...
        Assert.True(await harness.Consumed.Any<EditContactCommand>());

        mockContactService.Verify(
                s => s.EditAsync(It.Is<Contact>(c => c.Id == command.Id)), Times.Once());
